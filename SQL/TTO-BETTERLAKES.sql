...
UPDATE  Buildings SET FreeBuildingThisCity = 'BUILDING_OCEAN_MINING_DUMMY'
        WHERE BuildingClass = 'BUILDINGCLASS_OCEAN_MINING'
        AND EXISTS (SELECT * FROM COMMUNITY WHERE Type = 'TTO_AQUAMINING' AND Value = 1)
        AND EXISTS (SELECT * FROM Resources WHERE Type = 'RESOURCE_IA_LAKE_FISH');
