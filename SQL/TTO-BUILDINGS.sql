...
-- sadly, it's more powerful this way because your city doesn't need to work the mountains to benefit it
-- Building_YieldPerXTerrainTimes100
INSERT  INTO Building_YieldPerXTerrainTimes100 (BuildingType, TerrainType, YieldType, Yield)
SELECT	'BUILDING_AQUEDUCT', 'TERRAIN_MOUNTAIN', 'YIELD_FOOD', 100
        WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type = 'TTO_BLD_MT' AND Value = 1) UNION ALL
SELECT	'BUILDING_BROADCAST_TOWER', 'TERRAIN_MOUNTAIN', 'YIELD_CULTURE', 200
        WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type = 'TTO_BLD_MT' AND Value = 1) UNION ALL
SELECT	'BUILDING_BROADCAST_TOWER', 'TERRAIN_MOUNTAIN', 'YIELD_SCIENCE', 100
        WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type = 'TTO_BLD_MT' AND Value = 1) UNION ALL
SELECT	'BUILDING_BOMB_SHELTER', 'TERRAIN_MOUNTAIN', 'YIELD_CULTURE_LOCAL', 100
        WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type = 'TTO_BLD_MT' AND Value = 1) UNION ALL
SELECT	'BUILDING_BOMB_SHELTER', 'TERRAIN_MOUNTAIN', 'YIELD_PRODUCTION', 200
        WHERE EXISTS (SELECT * FROM COMMUNITY WHERE Type = 'TTO_BLD_MT' AND Value = 1);
